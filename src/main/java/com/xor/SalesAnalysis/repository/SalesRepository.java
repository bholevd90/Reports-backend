...
public interface SalesRepository extends JpaRepository<Sales, Integer> {
    ...
    @Query(value="select store_id,store_name,sum(weekly_sales_amount) from sales where holiday_flag=1 group by store_id,store_name",nativeQuery = true)
    List<Object[]> getHolidayAnalysis();

    @Query(value = "select store_id,store_name,sum(weekly_sales_amount) from sales where holiday_flag=0 group by store_id,store_name",nativeQuery = true)
